...
def get_all_files_in_path(path, prefixes=["test", "train"]):
    ...
    for prefix in prefixes:
        # Find files that start with the current prefix
        files_with_prefix = list(folder_path.glob(f"{prefix}*"))
        all_files.extend(files_with_prefix)
...
def main():
    main_dir_dataset = "/home/ubuntu/Project7008/data"
    processed_data_dir = '/home/ubuntu/Project7008/data/Processed'

    # Get file index from command line argument
    file_index = int(sys.argv[1]) if len(sys.argv) > 1 else 0

    # Retrieve all files (both train and test)
    all_files = get_all_files_in_path(main_dir_dataset)

    total_files = len(all_files)

    # Determine the file to process based on the file_index
    if file_index < total_files:
        files_to_process = [all_files[file_index]]
    else:
        print(f"No file assigned to CPU with index {file_index}")
        files_to_process = []
